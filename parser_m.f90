module parser_m
  ...
  private
  public :: parser_init,parser_parsefile,parser_is_defined, &
            parser_read,parser_getsize
...
contains
  ...
  subroutine parser_parsefile(input,iunit,istatus)
    
    implicit none
    integer :: ierr,limiter,nlines,i,j,ntags,comment
    ...
    if (ierr .ne. 0) then
       if(present(istatus)) then
          ...
          stop 'FATAL. Unable to open the input file.'
       end if
    end if
  ...
  subroutine parser_readlogical(mytag,value,default)
    ...
    if (.not.isdef .AND. present(default)) then   
       ...
       print*,'FATAL. parser : '// mytag //' not defined'
       stop
    else
